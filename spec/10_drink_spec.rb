...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_cocktail) { described_class.new('Caprinha') }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_cocktail.type).to eq('Caprinha')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_cocktail.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_new_cocktail) { described_class.new('Mojito') }
      # remove the 'x' before running this test
      it 'is full' do
        expect(my_new_cocktail).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_squash) { described_class.new('orange', 10) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_squash).not_to be_full
      end
    end
  end
end
