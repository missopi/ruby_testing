...
describe MagicSeven do
  ...
  describe '#divide_by_two' do
    context 'when previous step is 12' do
      it 'returns 6' do
        previous_step = 12
  ...
  subject(:new_game) { described_class.new(3) }
  ...
  describe '#play' do
    ...
    context 'when the random number is 25' do
      subject(:game_twentyfive) { described_class.new(25) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_twentyfive.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 1000' do
      subject(:game_onethousand) { described_class.new(1000) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_onethousand.play
        expect(result).to eq(7)
      end
    end
  end
